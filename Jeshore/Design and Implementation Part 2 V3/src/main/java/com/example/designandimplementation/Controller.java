...
public class Controller{
    ...
    private void treeViewClick(MouseEvent event){ /*When the tree view is clicked the function determines if it is a
        ...
        if (event.isSecondaryButtonDown()){
            ...
            } else if ((getSelectedItemParentOfParent() != null && getSelectedItemParentOfParent().getName().equalsIgnoreCase("farm"))) {
                addDrone.setDisable(true);
                addItem.setDisable(false);
                addContainer.setDisable(true);
                delete.setDisable(false);
    ...
    private void deleteMenu(){ //On menu click delete selected item or container and update list
        ...
        double deleteAmount = deleteItem.getPriceAll();
        ...
        if (child != null) {
            ...
            deleteAmount = totalValueOfChild;
            System.out.println(deleteAmount +" deleteAmount");
        }
        if (parent != null){
            System.out.println(deleteAmount +" deleteAmount2");
            System.out.println(parent.getPriceAll());
            parent.setPriceAll(parent.getPriceAll() - deleteAmount);
            System.out.println(parent.getPriceAll());
        }
        if(parentOfParent != null && !parentOfParent.getName().equalsIgnoreCase("farm")){
            System.out.println(deleteAmount +" deleteAmount3");
            parentOfParent.setPriceAll(parentOfParent.getPriceAll() - deleteAmount);
        }
    ...
    void scanFarm(){ //animates drone to scan the entirety of the farm
        ...
        simulatorButton.setDisable(true);
        launchButton.setDisable(true);
        simulatorButton.setText("Scanning Farm...");
